  ...
  public static void main(String[] args) {
    ...
    while (playerMoney > 0) {
      ...
      while (true) {
        ...
        } else if (response == 2) { // stand
          break;
        } else if (response == 3) { // split
          // check if can split
          if (playerHand.deckSize() > 2 || playerHand.getCard(0) != playerHand.getCard(1)) {
            System.out.println("You cannot split right now.");
            continue;
          }
          // deal new hand, then run loop on that hand until complete
          //Deck splitHand = new Deck();
          // TODO: Implement split functionality
          break;
        } else if (response == 4) { // double down
          while(true) {
            System.out.println("You can bet up to $"+bet+", enter a number for the additional amount you want to bet:");
            double newBet = userInput.nextDouble();
            if (newBet > playerMoney || newBet > bet) {
              System.out.println("You have bet too much, please bet within your means.");
              continue; 
            }
            else {
